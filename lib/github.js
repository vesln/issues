...
github.repos = function(username, cb) {
  ...
  github.request(url, function(err, response, body) {
    switch (response.statusCode) {
      case 404: return cb(new Error('(404) Oops. User not found.'));
      case 401: return cb(new Error('(401) Oops. Invalid login credentials.'));
      case 403: return cb(new Error('(403) Hit GitHub api limit. Try again after few secs.'));
    }
...
github.issues = function(username, repos, cb) {
  ...
  var issue = function(repo, done) {
    ...
    github.request(url, function(err, response, body) {
      switch (response.statusCode) {
        case 404: return done(new Error('(404) Oops. Repo not found.'));
        case 401: return done(new Error('(401) Oops. Invalid login credentials.'));
        case 403: return done(new Error('(403) Hit GitHub api limit. Try again after few secs.'));
      }
